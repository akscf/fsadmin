...
# https://akscf.me/
# ******************************************************************************************
package SwitchAdmin::DAO::SwitchRecordingDAO;
...
sub new ($$;$) {
    ...
    my $self = {
        ...
        class_name   => $class,
        fsadmin      => $pmod,
        base_path    => $pmod->get_config('freeswitch', 'recordings_path')
    };
    bless($self, $class);    
    unless($self->{base_path}) {
        die Wstk::WstkException->new("Missing property: freeswitch.recordings_path", RPC_ERR_CODE_INTERNAL_ERROR);
    }
    #    
    $self->{logger}->debug("switch recordings: ".$self->{base_path});
    #
