...
# https://akscf.me/
# ******************************************************************************************
package SwitchAdmin::DAO::SwitchSoundDAO;
...
sub new ($$;$) {
    ...
    my $self = {
        ...
        class_name   => $class,
        fsadmin      => $pmod,
        base_path    => $pmod->get_config('freeswitch', 'sounds_path')
    };
    bless($self, $class);    
    unless($self->{base_path}) {
        die Wstk::WstkException->new("Missing property: freeswitch.sounds_path", RPC_ERR_CODE_INTERNAL_ERROR);
    }
    #    
    $self->{logger}->debug("switch sounds: ".$self->{base_path});
    #
